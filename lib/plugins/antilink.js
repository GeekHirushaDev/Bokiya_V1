...
exports.cmdName = () => {
  return {
    ...
    category: "admin",
    alias: ['antilinkall'],
    desc: "Stop participants except admins to send links to the gc."
  };
}
...
exports.getCommand = async (userAdmin, args, botNumber, userOwner, botAdmin, groupMetadata, userSudo, command, prefix, anyaV2, pika) => {
  ...
  const done = await saveCase(pika.chat, args[0], 'antilink');
  if (done) {
   await anyaV2.sendMessage(pika.chat, {
      text: `\`\`\`⚠️ Warning!!\`\`\`\n\nAntilink has been turned on. Please ensure that users do not send links, otherwise they will be kicked out directly.`,
