module.exports = {
  ...
  getCommand: async (pika, BokiyaDev) => {
    ...
    if (/image/.test(mime) && !/webp/.test(mime)) {
      ...
      axios.post(dev.api.apiHubRaw + `api/sticker.js?key=${dev.api.apiHubKey}&packname=${packname}&author=${author}`, {
        ...
        .then((res) => res.data)
        .then(async (res) => await BokiyaDev.sendMessage(pika.chat, { sticker: Buffer.from(res.results.sticker, 'base64') }, { quoted: pika }))
        .catch((err) => {
          console.error(err);
      ...
      await BokiyaDev.sendMessage(pika.chat, { sticker: buffer }, { quoted: pika });
