module.exports = {
  ...
  getCommand: async (text, args, prefix, command, pika, BokiyaDev) => {
    ...
    if (args[0] && !args[1] && !args[0].includes('@')) {
      const { key } = await BokiyaDev.sendMessage(pika.chat, { text: Config.message.wait }, { quoted: pika });
...
`;
    ...
    const key = await BokiyaDev.sendMessage(pika.chat, { text: `🧐 Getting your ${vidQ.split('|')[0]} ${(vidQ.split('|')[1] === 'document') ? 'document' : 'video'}...` }, { quoted: pika });
    (vidQ.split('|')[1] === 'document') ? await module.exports.sendDoc(vidU, vidQ, key.key, pika, BokiyaDev) : await module.exports.sendVid(vidU, vidQ, key.key, pika, BokiyaDev);
  },
  sendVid: async (url, vidQ, key, pika, BokiyaDev) => {
...
╰────────────────┈✧
`;
    BokiyaDev.sendMessage(pika.chat, { video: await getBuffer((vidQ.split('|')[0] === '720') ? data.videoQuality.high : data.videoQuality.low), caption: caption }, { quoted: pika })
  ...
  sendDoc: async (url, vidQ, key, pika, BokiyaDev) => {
    const { getBuffer } = require('../lib/myfunc');
    const { tiny } = require('../lib/stylish-font');
    const YouTube = require('../lib/ytdl-core');
    const data = await YouTube.getVidUrl(url);
