...
exports.getCommand = async (args, pickRandom, prefix, text, command, BokiyaDev, pika) => {
  require('../../config');
  ...
  const starting = (alreadyIn)
                    ? await BokiyaDev.sendMessage(pika.chat, { text: '*✅ Surrendered Previous*', mentions: [pika.sender] }, { quoted: pika })
                    : await BokiyaDev.sendMessage(pika.chat, { text: 'Getting quize...', mentions: [pika.sender] }, { quoted: pika });
  ...
  const questionMsg = await BokiyaDev.sendMessage(pika.chat, { text: `_📌 Difficulty: ${quize.difficulty}_\n_📌 User: *@${pika.sender.split("@")[0]}*_\n\n*🌟 Question: ${quize.question.text}*\n\n\`\`\`Reply a number:\`\`\`\n${mixOptions}\n\n_ID: QA21_\n_UID: ${quize.id}_`, edit: starting.key, mentions: [pika.sender] });
  await sleep(500);
  const timingMsg = await BokiyaDev.sendMessage(pika.chat, { text: '🌈 You have only _*60*_ seconds', mentions: [pika.sender] }, { quoted: questionMsg });
  ...
  try {
    for (let i = countdownTime; i >= 0; i--) {
      ...
      await BokiyaDev.sendMessage(pika.chat, { text: `🌈 You have only _*${i}*_ seconds`, edit: timingMsg.key, mentions: [pika.sender] });
      if (i === 0) {
        await handleTimeUp();
      }
...
async function handleTimeUp() {
  await BokiyaDev.sendMessage(pika.chat, { text: `*Time's Up! You Lost @${pika.sender.split("@")[0]}* ❌⏳`, edit: questionMsg.key, mentions: [pika.sender] });
  await BokiyaDev.sendMessage(pika.chat, { delete: timingMsg.key });
...
//const userData = await isQuizeUserID(user.split("@")[0]);
 ...
 //   pika.reply('🎉 Correct!');
    BokiyaDev.sendMessage(pika.chat, { text: `*Correct! You Won @${user.split("@")[0]} 🎉🎊*`, edit: userData.question, mentions: [user], })
    .then((user) => { BokiyaDev.sendMessage(pika.chat, { delete: userData.timing }).then((id) => { return delQuizeUser(userID) }) });
    } else {
   BokiyaDev.sendMessage(pika.chat, { text: `*Wrong... You Lost @${user.split("@")[0]} 📉❌*`, edit: userData.question, mentions: [user], })
    .then((user) => { BokiyaDev.sendMessage(pika.chat, { delete: userData.timing }).then((user) => { return delQuizeUser(userID) }) });
        }
    }
}
