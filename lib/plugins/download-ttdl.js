module.exports = {
  ...
  getCommand: async (args, pika, BokiyaDev) => {
    ...
    const { key } = await BokiyaDev.sendMessage(pika.chat, { text: '```Downloading...```' }, { quoted:pika });
    const axios = require('axios');
    axios.get(`https://vihangayt.me/download/tiktok?url=${args[0]}`)
    .then(async (response) => BokiyaDev.sendMessage(pika.chat, { video: await getBuffer(response.data.data.play_url) }, { quoted:pika })
