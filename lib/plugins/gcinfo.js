...
const axios = require("axios");
  ...
  await pika.react("🔯");
  const wait = await BokiyaDev.sendMessage(pika.chat, { text: message.wait }, { quoted: pika });
  try {
    const groupId = text.split("https://chat.whatsapp.com/")[1];
    const gcstalk = await BokiyaDev.groupGetInviteInfo(groupId);
    let groupp = "https://i.ibb.co/ZKKSZHT/Picsart-23-06-24-13-36-01-843.jpg";
    try {
      groupp = await BokiyaDev.profilePictureUrl(gcstalk.id, 'image');
    } catch (e) {
    ...
    const context  = {
      ...
      caption: `*📌 ${tiny("Name")}:* ${gcstalk.subject}\n\n`
        ...
        + description,
      headerType: 4,
      mentions: gcstalk.participants.map(a => a.id).concat([gcstalk.owner ? gcstalk.owner : '94755681782@s.whatsapp.net'])
    };
    await BokiyaDev.sendMessage(pika.chat, context, { quoted: pika });
  } catch {
    return BokiyaDev.sendMessage(pika.chat, {
