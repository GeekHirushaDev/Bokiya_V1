...
exports.getCommand = async (text, mime, userOwner, userSudo, groupMetadata, BokiyaDev, pika, storage) => {
require('../../config');
...
${tiny(footer)}`;
       const mediaToBroad = await BokiyaDev.downloadAndSaveMediaMessage(pika.quoted ? pika.quoted : pika);
 ...
 if (/video/.test(mime)) {
  var sendMedia = {
   ...
   for (const broad of broadCastTo) {
    await BokiyaDev.sendMessage(broad, { ...sendMedia }, { quoted: pika });
    await sleep(500);
    }
