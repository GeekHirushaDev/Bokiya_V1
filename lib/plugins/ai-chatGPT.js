...
/**
exports.getCommand = async (text, BokiyaDev, command, pika) => {
  if (!text) return pika.reply(`Please provide some text or quote a message to get a response.`);
  const proceed = await BokiyaDev.sendMessage(pika.chat, { text: "📶 Getting response..." }, { quoted: pika });
  try {
...
exports.getCommand = async (text, pika, BokiyaDev) => {
 if (!text) return pika.reply("💭 Tell me something about your thoughts, I'm curious to answer.");
 const Config = require("../../config");
 const proceed = await BokiyaDev.sendMessage(pika.chat, { text: "🗣️ Generating response..." }, { quoted: pika });
