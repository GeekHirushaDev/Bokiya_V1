...
exports.getCommand = async (args, botAdmin, userAdmin, userOwner, userSudo, text, prefix, command, BokiyaDev, pika) => {
  ...
  const groupMetadata = pika.isGroup ? await BokiyaDev.groupMetadata(pika.chat) : pika.edit(Config.message.error, proceed.key);
  ...
  switch (command) {
    case 'invite':
      await invite(BokiyaDev, user, pika, groupMetadata, proceed);
      break;
    case 'add':
      await add(groupMetadata, user, proceed, BokiyaDev);
      break;
    case 'remove':
      await remove(BokiyaDev, pika, user, proceed);
      break;
   }
};
...
async function add(groupMetadata, users, proceed, BokiyaDev) {
  try {
  const isMulti = (users.length === 1) ? false : true;
    for (const user of users) {
      const exist = await BokiyaDev.onWhatsApp(user.split("@")[0]);
      if (exist.length === 0) {
      ...
      const action = await BokiyaDev.groupParticipantsUpdate(pika.chat, [user], 'add');
      const status = {
      ...
      const invited = `✨ Couldn't add *@${user.split("@")[0]}, send invitation!*`;
        ...
        await inviteV2(BokiyaDev, user, pika, groupMetadata, proceed);
      }
    }
  } catch (err) {
    console.error(err);
...
async function remove(BokiyaDev, pika, users, proceed) {
  const isMulti = (users.length === 1) ? false : true;
  try {
    for (const user of users) {
    const status = {
      ...
      : pika.edit(notFound, proceed.key, { mentions: [user] });
        ...
        const action = await BokiyaDev.groupParticipantsUpdate(pika.chat, [user], 'remove');
...
async function invite(BokiyaDev, users, pika, groupMetadata, proceed) {
  const isMulti = (users.length === 1) ? false : true;
    ...
    const already = `*🌊 @${user.split("@")[0]}*, is already added in this group.`;
      ...
      try {
        ...
        const unavailable = `❌ Can't find *@${user.split("@")[0]}*, maybe this number is not available on WhatsApp.`;
          ...
          const getLink = await BokiyaDev.groupInviteCode(pika.chat);
          let groupp = "https://i.ibb.co/ZKKSZHT/Picsart-23-06-24-13-36-01-843.jpg";
          try {
            groupp = await BokiyaDev.profilePictureUrl(pika.chat, 'image');
          } catch (e) {
            groupp = "https://i.ibb.co/ZKKSZHT/Picsart-23-06-24-13-36-01-843.jpg";
          }
          await BokiyaDev.sendMessage(user, {
...
async function inviteV2(BokiyaDev, user, pika, groupMetadata, proceed) {
  try {
    const getLink = await BokiyaDev.groupInviteCode(pika.chat);
    let groupp = "https://i.ibb.co/ZKKSZHT/Picsart-23-06-24-13-36-01-843.jpg";
    try {
