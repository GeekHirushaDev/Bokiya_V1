module.exports = {
  ...
  getCommand: async (text, pika, BokiyaDev) => {
    if (!text) return pika.reply('Enter a search term to search from Pinterest.com 📸');
    const { api } = require('../.dev');
    const limit = 5;
    const { getBuffer } = require('../lib/myfunc');
    const { key } = await BokiyaDev.sendMessage(pika.chat, { text: `🌄 Searching at least *${limit}* images...` }, {quoted:pika});
    ...
    api.apiHub('api', 'pinterest', `q=${encodeURIComponent(text)}`)
      .then(async (response) => {
        ...
        for (let pins of results.images) {
          min++;
          if (min === (max + 1)) break;
          await BokiyaDev.sendMessage(pika.chat, { image: await getBuffer(pins), caption: `_© Searched by ${Config.botname}_`, quoted: pika });
        }
      })
      .catch((error) => {
        console.error(error);
