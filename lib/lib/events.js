...
const eventSwitch = findSwitch();
    ...
    */
exports.eventListner = async (groupAction, BokiyaDev) => {
  try {
    const eventSwitch = await findSwitch();
   if (!eventSwitch.events) return;
   try {
     var groupProfile = await BokiyaDev.profilePictureUrl(groupAction[0].id, 'image');
   } catch {
     var groupProfile = 'https://i.ibb.co/ZKKSZHT/Picsart-23-06-24-13-36-01-843.jpg';
     }
  if (groupAction[0].announce == true) return BokiyaDev.sendMessage(groupAction[0].id, {
        image: { url: groupProfile },
        caption: "*✅ Action:* Group Closed\n*✨ Desc:* _No one can now send messages in this group, exept group admins._",
      });
   if (groupAction[0].announce == false) return BokiyaDev.sendMessage(groupAction[0].id, {
        image: { url: groupProfile },
        caption: "*✅ Action:* Group Opened\n*✨ Desc:* _Anyone can send messages in this group now._",
      });
   if (groupAction[0].restrict == true) return BokiyaDev.sendMessage(groupAction[0].id, {
        image: { url: groupProfile },
   ...
   if (groupAction[0].restrict == false) {
      BokiyaDev.sendMessage(groupAction[0].id, {
        image: { url: groupProfile },
...
exports.eventListner2 = async (groupEvent, BokiyaDev) => {
  try {
  const eventSwitch = await findSwitch();
  const metadata = await BokiyaDev.groupMetadata(groupEvent.id);
  const users = groupEvent.participants;
  try {
   var ppuser = await BokiyaDev.profilePictureUrl(users[0], 'image')
  } catch {
   var ppuser = 'https://i.ibb.co/ZKKSZHT/Picsart-23-06-24-13-36-01-843.jpg' };
  try {
   var ppgroup = await BokiyaDev.profilePictureUrl(groupEvent.id, 'image')
  } catch {
   var ppgroup = 'https://i.ibb.co/ZKKSZHT/Picsart-23-06-24-13-36-01-843.jpg' };
  if (groupEvent.action === 'promote') {
  if (eventSwitch.promote) return await BokiyaDev.sendMessage(groupEvent.id, {
           image: { url: ppuser },
           caption: Config.promote.replace('$user', users[0].split("@")[0]).replace('$prefix ', prefa),
           mentions: [users[0]]          
        });
   } else if (groupEvent.action === 'demote') {
  if (eventSwitch.demote) return await BokiyaDev.sendMessage(groupEvent.id, {
           image: { url: ppuser },
           caption: Config.demote.replace('$user', users[0].split("@")[0]).replace('$prefix ', prefa),
           mentions: [users[0]]          
        });
  } else if (groupEvent.action === 'add') {
  if (eventSwitch.welcome) return await BokiyaDev.sendMessage(groupEvent.id, {
           image: { url: ppuser },
  ...
  if (eventSwitch.goodbye) return await BokiyaDev.sendMessage(groupEvent.id, {
           image: { url: ppuser },
